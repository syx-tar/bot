/**
 * Remove Premium Status Command - Owner only
 * Usage: /delpremium <username>
 */

module.exports = {
  name: 'delpremium',
  description: 'Remove premium status from a user by username',
  ...
  async execute(message, args) {
    ...
    if (user.is_premium !== true) {
      await global.sendMessage(
        chatId,
        `*ℹ️ Info:* User *${user.first_name}* (@${user.username}) is not a premium user.`,
        { parse_mode: 'Markdown' }
      );
      return;
    }
    
    // Update user data to remove premium status
    const userData = {
      ...user,
      is_premium: false
    };
    ...
    try {
      ...
      if (currentDb.users && currentDb.users[targetUserId]) {
        currentDb.users[targetUserId].is_premium = false;
        fs.writeFileSync(DB_FILE, JSON.stringify(currentDb, null, 2), 'utf8');
        console.log(`✅ Premium status removed for user ${user.username}`);
      }
    } catch (error) {
    ...
    await global.sendMessage(
      chatId,
      `*✅ Success:* Premium status removed from *${user.first_name}* (@${user.username}).`,
      { parse_mode: 'Markdown' }
    );
  }
}; 